...
字段结构（`packages/core/src/config/settings.rs`）：

| 字段 | 类型 | 说明 |
|---|---|---|
| `cookie`, `stoken`, `uid`... | `String` | 认证凭据 |
| `shortcuts` | `ShortcutConfig` | 全局快捷键绑定 `BTreeMap<action_id, accelerator>` + 运行冲突列表 |
| `system` | `SystemConfig` | 系统集成：`left_click_toggle: bool` 控制托盘左键显隐 |
| `notification` | `NotificationConfig` | 通知规则（阈值、提醒时间、开关） |
| `data_dir` | `String` | 自定义数据目录（空=默认） |
| `first_run_done` | `bool` | 引导完成标记 |
